...
- **模型职责：**OpenRouter 兴趣模型是后台轻量判定与杂项初筛面，只处理有界的触发、分类、风险标注和简单审核；主动判断使用较高温度形成拟人波动，一致性敏感的初筛使用低温。Codex 主模型是聊天、总结与复杂推理面，负责语境、工具检索、选题、聊天/印象/人格总结、知识提取和最终回复。普通主动回复、冷群和私聊主动联系均由兴趣模型给最终开关，主模型不会再重复审批；携带长证据或需要复杂推理的后台杂项则由兴趣模型先初筛、主模型终审。
- **存储：**设置、记忆和社交状态保存在本地文件。`qq-knowledge-base` 已通过 repository 进行安全加载与原子写入；格式错误会保留原文件并切换只读保护，其他存储后续按小步继续抽取。
