...
这个项目同时涉及 Node.js、Codex 登录、QQ/OneBot、配置文件、后台进程和端口检查。公共安装器和仓库 `ncc` 会保留已有配置，但不同系统的路径、OneBot 实现和现有运行方式仍可能不同。让 Codex 作为部署操作员，可以先读机器现状，再选择最小、安全且可验证的动作。
...
脚本内可直接安装依赖、验证、填写配置并启动 Hub。然后准备 OneBot 并完成 QQ 登录；详细运行与排障见 [运维指南](OPERATIONS_CN.md)。
