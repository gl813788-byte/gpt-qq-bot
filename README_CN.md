...
Hub 会把每个群和私聊分别压缩成不含身份与私密原文的摘要，并在累计到足够消息和 Bot 回复后，用当前 QQ 回复模型生成全局 Bot 人设，名称强制等于登录 QQ 昵称。全局人设包含性格、自我描述、兴趣关键词、完整兴趣段落、加权兴趣和主动话题，保存在 `data/qq-self-persona.json`；各会话只贡献摘要，私聊原文不会被带到其他会话。默认首次会话摘要需累计 32 条消息，后续需新增 48 条真人消息或 12 次 Bot 回复且至少间隔 12 小时；全局人设首次需 80 条总消息及至少 2 个已摘要会话，后续需新增 160 条真人消息、40 次 Bot 回复或 8 份会话摘要且至少间隔 48 小时。真人与 Bot 的风格复盘同样放宽为每 48 小时一次。网页的 QQ 通道页会显示当前人设、关键词、生成进度和更新策略。
