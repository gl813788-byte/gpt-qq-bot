...
- This machine currently runs NapCat 4.18.9. Its public OneBot API supports request approval but does not expose an action that initiates a friend request or group join request. `ncc connect` deploys the loopback-only plugin from `modules/napcat-social-bridge` as `napcat-plugin-builtin` and configures `CODEX_REMOTE_CONTACT_QQ_SOCIAL_API_BASE`, so `/主动加好友 QQ号 验证语` uses the current QQNT single-object `ReqToFriend` signature and is supported locally. The bridge reports `submitted` or `already_friend` only after a successful upstream call. Active group join remains explicitly unsupported because `/join-group` returns `501`; never claim that it was sent.
