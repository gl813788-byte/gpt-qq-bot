...
- `/新对话`: starts a new QQ conversation by clearing this scope's lightweight memory, conversation transcript, `/记忆` short-term notes, pending image request, and group proactive state. In private QQ it clears only the current private scope. Old aliases such as `/清空上下文` remain supported.
...
- This machine currently runs NapCat 4.18.9. Its public OneBot API supports request approval but does not expose actions that initiate friend or group requests. `ncc connect` deploys the loopback-only plugin from `modules/napcat-social-bridge` as `napcat-plugin-builtin` and configures `CODEX_REMOTE_CONTACT_QQ_SOCIAL_API_BASE`. NapCat 4.18.9 declares the native friend action as `reqToAddFriends(uin, message)`; the bridge must use that two-argument shape (including when a native binding reports arity 0), select a correct-answer value for answer-based verification, and use a structured request object only when the runtime explicitly exposes a one-argument method. Remarks and categories are available only on that structured-object variant. `/主动加群` searches the group, reads its question and join mode, and submits the QQNT join request with the supplied answer and join authorization. Both paths detect already-friend/member, disabled requests, missing answers, full groups, reported native failures, and surfaced QQ risk-control errors. `submitted`/`pending_approval` means the native invocation completed without a reported error; it does not mean the peer or group has accepted. QQ Space actions use credentials obtained from NapCat and the currently deployed Tencent QZone web endpoints.
