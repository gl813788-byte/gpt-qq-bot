...
OneBot webhook / dashboard API
  ...
  -> app-server Codex turn + bounded Agent tools
...
| `src/codex-app-server-turn.js` | Run one controlled Codex app-server turn, start/resume a thread, and expose exact in-flight `turn/steer`/interrupt controls. |
| `src/qq-reply-steering.js` | Debounce many Bot-triggering follow-ups into one snapshot, steer once, and retain entries when the active turn cannot accept them. |
...
- A scope has one reply lifecycle at a time. Later messages enter the follow-up buffer only when routing says the Bot should reply: explicit mention/reply, approved interest, or another valid trigger. A resettable 900ms quiet window (2.5s maximum) fuses all such triggers, adjacent repeats, images, and context selected from between them into one `turn/steer` input. Do not resend full context per trigger. If the active turn closes first, revise the unsent draft with the fused batch before delivery when possible. `/stop` interrupts current work and clears only that reply's queue while retaining conversation context, short-term notes, and any reusable Codex thread; `/新对话` additionally clears the conversation-scoped state and thread mapping.
...
| Codex | `CODEX_CLI_PATH`, `CODEX_REMOTE_CONTACT_CODEX_MODEL`, `CODEX_REMOTE_CONTACT_REASONING_EFFORT`, `CODEX_REMOTE_CONTACT_CODEX_MAX_CONCURRENCY`, `CODEX_REMOTE_CONTACT_CODEX_MAX_PENDING`, `CODEX_REMOTE_CONTACT_QUOTA_CACHE_TTL_MS`, `CODEX_REMOTE_CONTACT_CODEX_*_TIMEOUT_MS`; `ai.*` | Default queue is 2 active and 32 pending. Task deadlines are classified independently: reply 2m, vision reply 3m, context summary 90s, self-persona 90s, file task 5m and image generation 10m. Model changes must use the live account model list and supported reasoning efforts. |
...
- Social conversation memory is persisted separately at `/root/Codex-Remote-Contact/data/qq-conversation-memory.json`. Version 2 keeps group impressions/topics scoped to their group while stable, non-sensitive person impressions are keyed by QQ id and shared across groups where that person appears. Current group cards are retained separately per group; raw group chat and group-private facts must never be copied into the cross-group person layer. Version-1 per-group person records migrate automatically. Private-chat memory remains separate. The agent may emit an invisible `[[qq_memory:{...}]]` patch after a reply; the Hub validates and strips it before QQ delivery. `/新对话` clears the rolling transcript/context but intentionally keeps these longer-lived impressions; explicit QQ memory-clear APIs clear both.
- Every recurring QQ domain behavior uses persisted wall-clock time rather than process uptime. `src/wall-clock-scheduler.js` performs an immediate catch-up check on Hub startup and another when the QQ channel is enabled, then polls only as a wake-up mechanism. An overdue task runs once instead of replaying every missed interval, and completion time becomes the next interval's anchor. Ordinary non-empty interest cycles persist their pending count, cycle start and bounded latest event in `data/qq-memory.json` version 3. Their restored event is marked for a one-time catch-up judge that bypasses the normal online stale-topic discard; new activity during that judge can still supersede its result. Knowledge-frequency review state persists in `data/qq-knowledge-base.json`; low-frequency deletion applications use bounded interest-model triage followed by full-evidence main-model final review. Adaptive style reviews, cold/private interest and self-persona summary/generation use the timestamps already stored in `data/qq-personas.json` and `data/qq-self-persona.json`. Manual `/总结聊天记录` and unified-memory read/write are event-driven, not recurring tasks. `/api/state` exposes the safe scheduler/runtime snapshot under `qq.periodic`.
...
- The QQ bot itself can use internal tools that are not shown in `/菜单`: it may emit `[[qq_command:/...]]` for permitted menu actions, `[[qq_command:/聊天记录 最近 50]]`, `[[qq_command:/聊天记录 20-40]]`, or `[[qq_command:/聊天记录 关键词]]` to inspect the current scope, and `[[qq_command:/联网 查询词]]` or `[[qq_command:/搜索 查询词]]` for web lookup. The hub supports an agent-style multi-round tool loop and strips `[[qq_done]]` before sending. Internal menu commands always use the original sender's permissions; they must never grant owner-equivalent authority. Any visible claim that a QQ write action completed—including poke, friend/group add, request handling, moderation, or QQ Space writes—must follow the corresponding tool call and an explicit successful result. The model may refuse before execution, but must report an already successful write and must never claim success after failure.
- Built-in social tools are also hidden from `/菜单`: `[[qq_command:/点赞 发送者 1]]`, `[[qq_command:/申请 列表]]`, `[[qq_command:/申请 同步]]`, `[[qq_command:/申请 同意 最新]]`, `[[qq_command:/申请 拒绝 #申请ID 理由]]`, `[[qq_command:/主动加好友 QQ号 验证=验证信息 | 答案=正确答案 | 备注=好友备注]]` (also `/加好友` and `/添加好友`), `[[qq_command:/主动加群 群号 答案=正确答案]]`, `[[qq_command:/动态 最近 QQ号 10]]`, `[[qq_command:/发动态 文字]]`, `[[qq_command:/发动态 图片=当前]]`, `[[qq_command:/发动态 文字 | 图片=当前]]`, and `[[qq_command:/评论动态 QQ号 tid 内容]]`. Image moods may use at most nine images from the current or quoted QQ message; arbitrary local paths are not accepted. Non-owner like targets are limited to the sender, mentioned users, and the replied-to user. Request handling, active add actions, and QQ Space writes are owner-only.
...
- This machine currently runs NapCat 4.18.9. Its public OneBot API supports request approval but does not expose actions that initiate friend or group requests. `ncc connect` deploys the loopback-only plugin from `modules/napcat-social-bridge` as `napcat-plugin-builtin` and configures `CODEX_REMOTE_CONTACT_QQ_SOCIAL_API_BASE`. NapCat 4.18.9 declares the native friend action as `reqToAddFriends(uin, message)`; the bridge must use that two-argument shape (including when a native binding reports arity 0). Stranger UID lookup is optional and must not block UIN submission. Use the loopback-only `/inspect-friend` route for read-only diagnostics; never turn a preflight into an unsolicited friend request. `/主动加群` searches the group, reads its question and join mode, and submits the QQNT join request with the supplied answer and join authorization. Both paths detect existing relationships, disabled requests, missing answers, full groups, reported native failures, and QQ risk control. `submitted`/`pending_approval` does not mean accepted. QQ Space actions use NapCat credentials and current Tencent QZone endpoints.
- OneBot poke notices are handled only when `target_id` is the bot's own QQ id. Other users poking each other, or bot-originated poke notices, are ignored. A poke targeting the bot is passed to the agent as a normal trigger. The prompt requires any visible claim such as “反拍/拍回去/拍回来/戳回去” to first emit `[[qq_command:/拍一拍 发送者]]` (or `/拍一拍 QQ号`) and wait for a successful tool result; failure or omission must not be described as completed. NapCat poke calls include both `user_id` and `target_id` and preserve both endpoint errors for diagnosis. As a deterministic fallback, the Hub also recognizes common missed-command phrases such as “反拍一下”, “拍回来”, and “反手戳” and performs the poke before retaining that visible claim.
- `/记忆` is current-scope short-term memory stored in `data/qq-memory.json`; internal tools support list/search/add/edit/delete/clear, and `/新对话` clears it. Long-term QQ memory is `/root/Codex-Remote-Contact/data/qq-knowledge-base.json`: every item has a title, ordinary notes may use any permitted scope, and slang stores a term plus explanation scoped globally, by group, by QQ person, or by group-person. Group name/id and person nickname/QQ id remain as scope identity. Same-person identical interpretations across groups promote to one person variant, while different group meanings remain isolated. Ordinary group knowledge must be derived from that group's actual recurring topics rather than any fixed catalog: each scope summary combines its previous summary/topics with new chat to evolve those topics, and the main reply model receives the current scope summary as weak knowledge-selection context. For changing external facts, use a stable subject title and content containing an as-of date, verification state, fact, and source; formatted notes expose their update timestamp. A tool-capable main-model turn checks the old title and current web evidence when the topic recurs or freshness matters, then overwrites the same title+scope. A summary task without web tools may only write the chat claim as pending verification, while non-public group rules are labeled as group agreement/consensus. The model can list/search/view titles and write via hidden `/知识库` tools or `[[qq_knowledge:{...}]]`; existing title+scope content is replaced rather than appended. Legacy `qq-public-memory.json` entries import into the knowledge base at startup.
...
- Interest reply also has timed group and private branches. Each group/contact learns a shortest circular 6–18 hour activity window covering at least 85% of observed messages; fewer than 20 samples use `09:00-23:00` Asia/Shanghai only as fallback. The cold-group branch requires at least 20 learned human samples and a quiet threshold of roughly 4/6/10/8 hours for high/typical/low/unknown activity. When due, the interest model returns `shouldStart`, `mode` (`silent`, `topic`, or `chatter`), `interest`, and `reason`; mode and start must agree. Approval launches the main model, which executes `topic` through optional multi-round research or `chatter` as rare lightweight presence. The interest model never selects the topic, query, draft, or style. The private branch sends its learned probability plus a fresh variation roll to the interest model as non-binding rhythm evidence; the model makes the final start decision, and only approval launches the main model to write one natural message. A decline, failure, missing key, or disabled judge never starts the main model. Delivered Bot bubbles still increase unanswered suppression. New activity supersedes stale generation. `/api/state` exposes the learned plans, and structured `interest` logs retain cold/private gates, temperature, bounded output, research details, and final outcomes.
...
- QQ image generation and owner file/image tasks use a per-request workspace under `runtime/qq-task-workspaces/<timestamp-kind-id>/` with `input/` for downloaded QQ images and `output/` for deliverable assets. Explicit generation or reference-edit wording (including edit/modify, background replacement, element addition/removal, style changes and drawing from a reference) takes this image-task path before the broad ordinary “look at this image” check. With a current or quoted image, a short mutation such as “把背景换成蓝色” also passes that image to the drawing model as a reference. The Hub only sends files whose real path remains under the current task's `output/`; images may additionally use the legacy `runtime/qq-output-images/` or local sticker directory. Markers pointing at task inputs, old workspaces, project files, arbitrary paths, or symlink escapes are rejected. An owner task that needs to send an existing local file must first copy it into this request's `output/`. Cleanup is a direct, path-validated recursive removal after delivery, not another Codex task. If an image marker has no permitted readable image, QQ receives an explicit failure instead of a false “已生成”.
- Hub-launched Codex work uses task-specific wall-clock limits from `src/config/environment.js`: ordinary reply, vision reply, context summary, self-persona, owner file task and image generation each have an independent `CODEX_REMOTE_CONTACT_CODEX_*_TIMEOUT_MS` setting. Image generation defaults to 10 minutes and may be configured up to 60 minutes; it does not inherit the shorter reply limit. `/api/maintenance`, `/详细配置` and Codex logs expose the selected policy, task type and deadline.
