...
- QQ image generation is supported opportunistically through the image/file task channel when a working image endpoint/tool is available. The backend validates that generated `[[qq_image:/path]]` markers point to real local image files before claiming success; if the first model pass has no sendable file, the backend gives Codex one repair pass with the error and previous reply so it can continue generating or补发. If the repair pass still has no real file, QQ should receive an explicit failure message instead of a text-only “已生成”. When diagnosing image-send bugs, check the reply file, retry reply file, `runtime/qq-output-images`, and whether the generated marker path existed before send cleanup.
