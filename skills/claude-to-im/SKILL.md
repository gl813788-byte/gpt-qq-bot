...
- The recommended public install is `npx -y "codex-qq-bot@$(npm view codex-qq-bot@latest version --prefer-online)"` or the same exact-version pattern with `pnpm dlx`; when Node.js is absent, use the raw-main `install.sh` curl bootstrap. The npm installer revision is `1.1.7-3`. Resolving the exact registry version before npx execution bypasses a stale `_npx` executable cache. The installer refreshes the repository default branch and exact latest commit through the GitHub API on every run, then downloads that commit-pinned source ZIP instead of waiting for a GitHub Release. Valid stages for the same commit remain resumable; a damaged cached ZIP is quarantined and fetched again, resume failures fall back to a complete download, and extraction is atomically rebuilt from a clean temporary directory. New installs default to `/root/Codex-QQ-Bot` or `~/Codex-QQ-Bot`, while an existing legacy `/root/Codex-Remote-Contact` installation is reused. A prior archive install without Git is upgraded through prepared new source that retains `data`, `runtime`, local configuration, and extra files, with the full old directory kept under the install cache's `backups/`; Git worktrees and unrelated occupied directories are not overwritten. Before exiting, the installer creates an `ncc` wrapper that enters the repository launcher when no different same-name controller exists, then explicitly tells the user to run `ncc`; it does not open the wizard by default. The repository `ncc` continues to own environment preparation, dependencies, `npm run verify`, guided configuration, and the persisted first-run state. QQ/NapCat binaries are not bundled or silently installed.
...
- Separate server snapshots from local interaction state: polling must not overwrite active controls, in-flight mutations, dirty forms, or the open/closed state of memory and adaptive-learning details. Preserve only non-secret, current-tab dashboard drafts, disclosure state, and browsing context in `sessionStorage`; restore them after a full reload, retain failed form drafts for retry, and clear a draft after a successful server save.
