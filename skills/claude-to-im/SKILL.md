...
- QQ image generation is currently not being pursued. The backend may route image-generation requests into the image/file task channel, but the current OpenAI-compatible gateway has returned `PermissionDeniedError: Your request was blocked` for Images API calls. Do not spend time trying to repair QQ drawing unless the user explicitly reopens that task or provides a working image endpoint.
